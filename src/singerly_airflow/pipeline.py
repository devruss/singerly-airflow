...
class Pipeline:
    ...
    def execute(self) -> None:
        ...
        print(
            f"Starting pipeline execution {self.get_tap_executable()} -> {self.get_target_executable()}"
        )
        ...
        while True:
            try:
                ...
                tap_log_line = tap_process.stderr.readline().decode("utf-8").strip()
                if tap_log_line:
                    print(tap_log_line)
                target_log_line = (
                    target_process.stderr.readline().decode("utf-8").strip()
                )
                if target_log_line:
                    print(target_log_line)
                if not next_line:
                    break
